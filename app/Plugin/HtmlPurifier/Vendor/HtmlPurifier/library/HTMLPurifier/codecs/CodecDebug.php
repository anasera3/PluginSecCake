...
class HTMLPurifier_CodecDebug
{
    ...
    {
        $this->_enabled
            = HTMLpurifier_ESAPI::getSecurityConfiguration()->getSpecialDebugging();
    }
    ...
    {
        if (!self::$_instance) {
            self::$_instance = new HTMLPurifier_CodecDebug();
        }
    ...
    public function addEncodedString($stringNormalizedEncoding)
    {
        if (   $this->_enabled == false
            || ! HTMLPurifier_ESAPI::getAuditor(CD_LOG)->isDebugEnabled()
            || ! $this->_allowRecurse
        ) {
            return;
        }
